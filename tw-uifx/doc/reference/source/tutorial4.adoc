...
other_di = 0.0;
    for (int i = 0; i < other_x.size(0); i++)
		if (other_x.getByInt(i) > 0.0)
			other_di -= (other_x.getByInt(i) / xtot) * log(other_x.getByInt(i) / xtot);
---------------------

==== User interface

We now need to modify the user interface employed by the previous model (Tut3) to display `di` and `tsd`.
...
The axis order of the xy plot 'diversity time since dist` can be set using the appropriate dialog from the widget menu in _ModelRunner_.

[#fig-tut4-xyplot-dlg]
.The user-interface sub-tree for Tutorial 4. .
image::tutorial4IMG/fig-tut4-xyplot-dlg.png[align="right",role = "thumb"]

*TODO not confident of this model - reversing order of Growth and Effects gives very different results*
