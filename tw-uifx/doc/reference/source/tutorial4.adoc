...
As the title suggests, in this tutorial we are elaborating the model structure. The Lotka-Volterra model has the simplest of structures: so simple that its `System` node can be named `community`. In the model we are developing here, this name would be misleading since we will have not only a 'community' but also a 'disturbance'. Therefore it will be clearer if we change the name of the system node back to its default: `sys1`.
...
[start = 9]
. Create a `field` of type `Double` as a child of `record:commDrvs`, add the `description` property and enter 'Time since disturbance' for the property value.

That's all the data needed for now. We can now specify the system dynamics.
