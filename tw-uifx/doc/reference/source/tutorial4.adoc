...
. Copy and paste the code below to all the relevant `javaCode` properties. A convenient way to do this is to enter "javaCode" into the search field in the _All properties_ property editor and edit each in turn. If you received compile error messages, check that you have pasted into the property of the correct function:
...
We now need to modify the user interface, provided curtesy of the previous model (Tut3), to display `di` and `tsd`.
...
.User-interface properties for Tutorial 4.
