...
. Delete `record:decs`. This record was provided by the template and is not needed here.
...
. Create a second `table` as child of `record:cnsts`, name it 'K' and choose `dataElementType` and then set its type to `Double`.
...
There are now four tasks in the task list indicating we have yet to associate the dimensioner with the four tables. _K_, _r_ and _x_ have one dimension and _alpha_ has two.
