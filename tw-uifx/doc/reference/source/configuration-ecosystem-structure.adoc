...
image::configuration-ecosystem-structureIMG/example-category.png[align="center"]
...
image::configuration-ecosystem-structureIMG/example-life-cycle.svg[align="center"]

is specified with this graph:

[#fig-life-cycle-config-example]
.Example of a life cycle configuration
image::configuration-ecosystem-structureIMG/example-life-cycle.png[align="center"]
...
A second method to set descriptor values is to import data from an external source. This is done by defining a  <<configuration-experiment.adoc#trueinputs-datasource,`dataSource`>> and linking a `component`, `group`, `lifeCycle` or `system` to it:
