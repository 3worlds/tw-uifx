...
This node is used when reading data into <<configuration-dataDefinition.adoc#truetable,tables>>. The node name must be an integer matching the dimension declared in a table under the `dataDefinition` node. It must then have the following property:
...
// TODO: redraw this image

[#fig-data-source-example]
.Example of a data source configuration.
image::configuration-experimentIMG/example-data-source.png[align="center"]
