...
The default location for installation is the user's home directory. This can be changed by setting an environment variable called 'TW_HOME'. Create a directory called `3w` in your chosen location directory and copy `tw<os name>.jar` to it; where `os name`is one of `linux`, `mac` or `win`.
