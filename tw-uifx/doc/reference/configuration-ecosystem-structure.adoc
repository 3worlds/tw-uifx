...
A second method to set descriptor values is to import data from an external source. This is done by defining a `dataSource` (cf. **REF**) and linking a `component`, `group`, `lifeCycle` or `system` to it:
