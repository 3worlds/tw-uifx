...
_possible values_:
[horizontal]
`ControlWidget1`:: A simple simulator controller with run/pause, step and reset buttons. Add as a child of a `top` or `bottom` node. A GUI definition can only have one controller.

`ControlWidget2`:: As `ControlWidget1`, but also displays stopping condition and the time taken to complete each simulation step and the total simulation time. These timings are only accurate when deploying a single simulation. They are not accurate when deploying multiple simulations. This widget is best used when developing a model to check its performance. ControlWidget1 should be used otherwise.

`ControlWidget3`:: As `ControlWidget2`, but displays a time series plot of simulation execution time. Add as a child of a `tab` or `container` node. 

`ProgressWidget1`:: A simple display of simulation time (average time if deploying multiple simulations) and stopping condition. Add as a child of a `top` or `bottom` node.

`ProgressWidget2`:: A bar graph of simulation progress intended for use when deploying multiple simulations. Add as a child of a `tab` or `container` node.

`TimeseriesWidget1`:: A graph of _x~i~_ = __f__(__t__), where _x~i~_ are component descriptors and t is time.  Add as a child of a `tab` or `container` node. This widget uses the fast charting library from Chartfx. Data can be view as a table and exported to file.

`ScatterplotWidget1`:: A graph of _y_ = __f__(__x__) where _x_ and _y_ are two descriptors of a component.  Add as a child of a `tab` or `container` node. This widget also uses the fast charting library from Chartfx.

`SpaceWidget1`:: A map of a space (<<configuration-ecosystem-structure.adoc#truethe-representation-of-space>>).  Add as a child of a `tab` or `container` node.
`TableWidget1`:: A table of number values __x~i~__(__t__).  Add as a child of a `tab` or `container` node. This widget is intended for use when debugging as much of its function is duplicated in widgets using the Chartfx library.   Add as a child of a `tab` or `container` node. 
...
`HLSimpleTimeSeriesWidget`:: Saves time series data to a file. Add as a child of a `headless` node. 
