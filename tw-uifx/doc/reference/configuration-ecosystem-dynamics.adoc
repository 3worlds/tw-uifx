...
An `EventTimer` maintains a queue of time events that gets populated by user-defined ecological <<truefunction,functions>>. This way, future events depend on the dynamics of the system. Functions that can populate an event queue will have an `EventQueue` argument in their heading, usable to create future time events. Events must set a time in the future, i.e. their time must be larger than current time.
