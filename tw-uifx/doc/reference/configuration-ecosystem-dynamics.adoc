...
CAUTION: We recommend not to use it, or only for simple models, since it may cause problems when the code contained in a snippet contains compile errors, something very easy to achive when, e.g., renaming oa field or table. If you use it, do it with caution.
...
IMPORTANT: A `dataTracker` node can only be specified in a _process_ that applies to __categories__, not to a __relation type__.
