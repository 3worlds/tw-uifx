...
Here, the component's _role_ is defined as: a _permanent atomic_ residing in an _arena_. It uses _r_ as a constant and _x_ as a driver and the process that applies to it is `process:p1`. 

The component is _permanent_ because it doesn't die, it's an _atomic_ simply because it is the only one, and it resides in an _arena_ because, well, everything's got to be somewhere. No matter how many components a model has, exactly one of them must belong to an _arena_, a unique top level component - its more or less a global component accessible to all other components. 
