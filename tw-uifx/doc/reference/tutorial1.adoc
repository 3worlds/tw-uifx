...
This tutorial explains the procedure for creating a simulation with _ModelMaker_. As the use of _ModelMaker_ is the primary focus of this tutorial, a simple logistic equation is implemented as a test case. New modelling capabilities are gradually introduced over the course of these tutorials.
...
Having created `Tut1`, the task list shows four nodes are required. This list can be dealt with in any order but in general, a logical approach is to build the specifications in the order: 
...
Here, the component's _role_ is defined as: a _permanent individual_ residing in an _arena_. It uses _r_ as a constant and _x_ as a driver and the process that applies to it is `process:p1`. 

The component is _permanent_ because it doesn't die, it's an _individual_ simply because it is the only one, and it resides in an _arena_ because, well, everything's got to be somewhere. No matter how many components a model has, exactly one of them must belong to an _arena_, a unique top level component - its more or less a global component accessible to all other components. 
...
The above edits have created three _cross-link_ edges. All _cross-links_ are red - thin at the _start node_ and thick at the _end node_ and are named. The relationship can be read as: `system:sys1 belongs to category:*arena*`.
...
You can also change font sizes, node sizes and node and edge names by hiding or showing either or both roles and names.
