...
. Create `record` node as a child of `dataDefinition:dDef` and name it 'cst'.

. Create a `field` node as child of `record:cst`, name it 'r' and select its type as `Double`.

. Create another `record` as child of `dataDefinition:dDef` and name it 'drv'.

. Create a `field` node as child of `record:drv`, name it 'x' and select the type `Double`.
