...
Even though we started this project from a "Blank" template, a sub-tree has already be added and hidden from view when the project was created. To view this sub-tree, right-click on root node (`3worlds:Tut1`) and select `expand -> predefined:*categories*`.
...
The next task is essentially the same thing but expressed slightly differently. Its asking that we connect an edge for the system node to either a node named *individual* or *population*. 

. Repeat the above steps and select `New edge->belongsTo->category:*individual*`.

The meaning of these connections will be more apparent when developing more complex models. We can say here that _3Worlds_ handles hierarchical systems and our single equation is understood as an "individual" simply because it is a single entity not made of parts. In a more complex model, biological and physical entities can be modelled as a population of parts.(*???*)
...
. *Dynamics*: In this sub-tree we create, in order, nodes called `timeLine`,`timer`, `process`, `function` and `dataTracker`. We will assume the default name is excepted in every case unless otherwise noted.
...
. Create a function node as a child of `process:p1`. After naming the function a prompt appears for the funtion type. Select the first option `ChangeState`. You cannot change the function type after the node is created. If you've made a mistake, delete the node and recreate it.

The logistic equation we're going to implement is x(t+1) = rx(t)/(1-x(t). To view the value of x over time we use a dataTracker connected to `process:p1`. 
...
Before creating the entity to function operates on we should define the model's data: in this case it is simply r and x.
...
. Create a field node as child of `record:par` and name it `r` and select its type a `Double`.
...
This is all the data we need to define. To arrange the display in an orderly way click the `L` at the bottom of the graph dispay.
