...
Decorators are values derived from the current value of the drivers. They are set to zero at the begining of each time step. Here, the _decorator_  will be a species diversity index (Shannon's), derived from relative abundance of each species. 
