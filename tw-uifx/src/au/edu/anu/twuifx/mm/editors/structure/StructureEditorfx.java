...
public class StructureEditorfx extends StructureEditorAdapter {
	...
	public StructureEditorfx(NodeEditor n, MouseEvent event, MMController controller, GraphVisualiser gv,
		...
		duration = GraphVisualiserfx.animateSlow;
	}
	...
	public void buildgui() {
		...
		addNewNodeEntry(cm, filteredChildSpecs);

		addNewEdgeEntry(filteredEdgeSpecs);

		addNewChildLinkEntry(orphanedChildren);
		...
		addOptionalPropertiesEntry(optionalNodePropertySpecs, optionalEdgePropertySpecs);
		...
		addImportTreeEntry(filteredChildSpecs);

		addExportTreeEntry();
	}

	private void addExportTreeEntry() {
	...
	private void addImportTreeEntry(List<SimpleDataTreeNode> filteredChildSpecs) {
		{
	...
	private void addRenameEdgeEntry() {
...
//				}
	...
	private void addOptionalPropertiesEntry(List<SimpleDataTreeNode> optionalNodePropertySpecs,
		...
		if (!nodeEditor.isPredefined() && (!optionalNodePropertySpecs.isEmpty())
			...
			mi.setOnAction((e) -> {
				...
				String desc = MenuLabels.ML_OPTIONAL_PROPS.label() + " [" + nodeEditor.toString() + "]";
	...
	private void addDeleteTreeEntry() {
	...
	private void addDeleteChildLinkEntry() {
		...
		if (nodeEditor.hasChildren() && !nodeEditor.isPredefined()) {
			for (LayoutNode child : nodeEditor.getChildren()) {
				...
				mi.setOnAction((e) -> {
					...
					String desc = MenuLabels.ML_DELETE_CHILD_EDGE.label() + " [" + child.configNode().toShortString()
							+ "]";
	...
	private void addDeleteEdgeEntry() {
		...
		if (nodeEditor.hasOutEdges()) {
			for (LayoutEdge edge : nodeEditor.getOutEdges()) {
				...
				mi.setOnAction((e) -> {
					...
					String desc = MenuLabels.ML_DELETE_EDGE.label() + " [" + nodeEditor.toString() + "]";
					recorder.addState(desc);
				});
			}
	...
	private void addDeleteNodeEntry() {
		MenuItem mi = MenuLabels.addMenuItem(cm, MenuLabels.ML_DELETE_NODE);
		if (!nodeEditor.is3Wroot() && !nodeEditor.isPredefined()) {
			mi.setOnAction((e) -> {

				String desc = MenuLabels.ML_DELETE_NODE.label() + " [" + nodeEditor.toString() + "]";
		...
		if (nodeEditor.hasUncollapsedChildren()) {
			...
			if (count > 1) {
				...
				mi.setOnAction(e -> {
					...
					String desc = "Collapse " + MenuLabels.ML_ALL.label() + " [" + nodeEditor.toString() + "]";
					recorder.addState(desc);
				});
			}
		} else
		...
		if (nodeEditor.hasCollapsedChild()) {
			...
			if (count > 1) {
				...
				mi.setOnAction((e) -> {
					...
					String desc = "Expand " + MenuLabels.ML_ALL.label() + " [" + nodeEditor.toString() + "]";
					recorder.addState(desc);
				});
			}
		} else
	...
	private void addNewChildLinkEntry(List<LayoutNode> orphanedChildren) {
	...
	private void addNewEdgeEntry(List<Tuple<String, LayoutNode, SimpleDataTreeNode>> filteredEdgeSpecs) {
	...
	private void addNewNodeEntry(ContextMenu cm2, List<SimpleDataTreeNode> filteredChildSpecs) {
